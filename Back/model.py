...
def preproc(target_image):
    ...
    images.append(cv2.resize(img ,dsize= (resize,resize), interpolation = cv2.INTER_LINEAR))
...
def create():
    ...
    conv1 = keras.layers.Conv2D(filters = 16, kernel_size=[3,3], padding='SAME', activation=tf.nn.relu)(bn)
    pool1= keras.layers.MaxPool2D(padding='SAME')(conv1)
    
    bn = keras.layers.BatchNormalization()(pool1)
    conv2= keras.layers.Conv2D(filters = 32, kernel_size=[3,3], padding='SAME', activation = tf.nn.relu)(bn)
    pool2 = keras.layers.MaxPool2D(padding='SAME')(conv2)

    bn = keras.layers.BatchNormalization()(pool2)
    conv3 = keras.layers.Conv2D(filters = 64,kernel_size=[3,3], padding='SAME', activation = tf.nn.relu)(bn)
...
js["accuracy"] = round(acc[0][0]*100,2)
js1["accuracy"] = round(acc[0][1]*100,2)
...
with open("results/result.json","w",encoding="utf-8") as json_file:
    json.dump(jslist, json_file,ensure_ascii=False)
